...
- **Pontos negativos:**
  ...
  - **Camada de infraestrutura ainda mais ampla que o necessário para o baseline.** O escopo pede "restart policies, logs e métricas" e "health checks". O repositório já traz Vault + External Secrets Operator, Elasticsearch + Kibana, Fluentd + Promtail, Jaeger, Prometheus + Alertmanager, OTEL Collector, cert-manager/ClusterIssuer, políticas de auditoria de API server e RBAC por serviço. Esses componentes agora ficam melhor posicionados como camada estendida/opcional do que como núcleo obrigatório do módulo, para não sobrecarregar o fluxo principal de Kubernetes.
...
Há ainda um **desequilíbrio de prioridades** que merece reflexão. A infraestrutura avançada continua presente, mas a documentação agora separa melhor o baseline do módulo e a camada estendida: o núcleo fica em aplicações, health checks e observabilidade mínima, enquanto Vault, External Secrets, ELK, Jaeger, Alertmanager e auditoria deixam de ser apresentados como obrigação do caminho principal. Enquanto isso, os segredos reais continuam em texto puro **commitados** em dois lugares (`.env` rastreado no Git e `secrets.yaml`), e o provisionamento de banco no compose segue com inconsistências (migrações desativadas + `InitDB.sql` montado num caminho que a imagem do SQL Server ignora, criando um banco que nenhum serviço usa).
